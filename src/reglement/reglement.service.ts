...
export class ReglementService {
  ...
  private async generatePaymentReceipt(data: {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        doc.fontSize(8).font('Helvetica');
        let paymentDetailY = clientInfoTop;
        ...
        // Afficher les détails selon le mode de paiement
        if (data.caisse) {
          doc.text(
        ...
        if (data.compte) {
          ...
          if (data.compte.banque) {
            doc.text(
              `Banque: ${this.sanitize(data.compte.numero_compte || 'N/A')}`,
              paymentInfoX,
              paymentDetailY,
            );
            paymentDetailY += 12;
        ...
        if (data.nita) {
          doc.text(
            `NITA: ${this.sanitize(data.nita.nom || 'N/A')}`,
            paymentInfoX,
            paymentDetailY,
          );
          paymentDetailY += 12;
          ...
          doc.text(
            `N° Compte: ${this.sanitize(data.nita.nom || 'N/A')}`,
            paymentInfoX,
            paymentDetailY,
          );
          paymentDetailY += 12;
  ...
  async createReglement(
    ...
    // Récupérer caisse, compte et nita si nécessaire
    let caisse: Caisse | null = null;
    let compte: Compte | null = null;
    ...
    if (id_nita) {
      nita = await this.nitaRepository.findOne({ where: { id_nita } });
      if (!nita) {
        throw new NotFoundException(`NITA avec l'ID ${id_nita} non trouvé`);
      }
    }
    ...
    if (id_commandes_vente && id_commandes_vente.length > 0) {
      ...
      if (facturesExistantes.length === 0) {
        throw new BadRequestException(
          `Aucune des factures spécifiées n'existe pour ce client`,
        );
      }
      ...
      if (missingIds.length > 0) {
        throw new BadRequestException(
          `Les factures suivantes n'existent pas: ${missingIds.join(', ')}`,
        );
      }
      ...
      if (facturesValides.length === 0) {
        throw new BadRequestException(
          `Toutes les factures spécifiées sont déjà entièrement réglées`,
        );
      }
    ...
    if (prioritizedFactures.length === 0) {
      throw new BadRequestException(
        'Aucune facture disponible pour ce règlement',
      );
    }
    ...
    for (const facture of prioritizedFactures) {
      ...
      reglement.id_compte = id_compte || null;
      reglement.compte = compte || null;
      reglement.id_nita = id_nita || null;
      reglement.nita = nita || null;
    ...
    if (caisse) {
      ...
      mouvementCaisse.solde_avant = soldeCaisseAvant;
      mouvementCaisse.solde_apres = caisse.solde;
      mouvementCaisse.libelle = `Règlement client ${client.nom || client.id_client} - ${facturesAffectees.length} facture(s)`;
    }
    ...
    if (compte) {
      ...
      mouvementCompte.solde_avant = soldeCompteAvant;
      mouvementCompte.solde_apres = compte.solde;
      mouvementCompte.libelle = `Règlement ${typeReglement.type_reglement} - Client ${client.nom || client.id_client}`;
      mouvementCompte.numero_piece =
        typeReglement.type_reglement === 'D' ? 'Chèque à définir' : null;
    }
    ...
    if (nita) {
      ...
      mouvementNita.id_nita = nita.id_nita;
      mouvementNita.nita = nita;
      mouvementNita.type_mouvement = 'CREDIT';
      mouvementNita.montant = montant;
      mouvementNita.date_mouvement = new Date(date);
      mouvementNita.type_operation = 'REGLEMENT';
      mouvementNita.solde_avant = soldeNitaAvant;
      mouvementNita.solde_apres = nita.solde;
      mouvementNita.libelle = `Règlement NITA - Client ${client.nom || client.id_client} - ${facturesAffectees.length} facture(s)`;
      mouvementNita.libelle = `NITA-${Date.now()}`;
    }
    ...
    try {
      await this.dataSource.transaction(async (transactionalEntityManager) => {
        ...
        if (nita && mouvementNita) {
          await transactionalEntityManager.save(Nita, nita);
          await transactionalEntityManager.save(MouvementNita, mouvementNita);
        }
        ...
        if (mouvementNita && savedReglements.length > 0) {
          mouvementNita.id_reglement = savedReglements[0].id_reglement;
          await transactionalEntityManager.save(MouvementNita, mouvementNita);
        }
      });
      ...
      if (nita) {
        console.log(
          `✅ NITA ${id_nita} mis à jour: solde=${nita.solde} (mouvement enregistré)`,
        );
      }
      console.log(
        `✅ Client ${id_client} mis à jour: solde=${client.solde}, avance=${client.avance}`,
